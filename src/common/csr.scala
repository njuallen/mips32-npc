...
class MStatus extends Bundle {
  ...
  /* The SD bit is a read-only bit that summarizes 
   * whether either the FS field or XS field signals the
   * presence of some dirty state that will require 
  ...
  val mprv = Bool() // Modify PRiVilege

  /* The FS[1:0] read/write field and the XS[1:0] read-only field 
   * are used to reduce the cost of context save and restore 
   * by setting and tracking the current state of the floating-point unit and 
   * any other user-mode extensions respectively. 
   */
  val xs = UInt(2.W)
  val fs = UInt(2.W)
...
object CSR
{
  ...
  // clear bits
  val C = 3.asUInt(SZ)
  // system instructions, do not touch csr registers
  val I = 4.asUInt(SZ)
  // only read cpu general register, do not write
  val R = 5.asUInt(SZ)
...
class CSRFileIO(implicit conf: SodorConfiguration) extends Bundle {
  ...
  // whether it's an environment return
  ...
  // exceptions detected by ALU/LSU
...
class CSRFile(implicit conf: SodorConfiguration) extends Module
{
  ...
  // after reset, we run in Machine mode
  reset_mstatus.mpp := PRV.M
  reset_mstatus.prv := PRV.M
  val reg_mstatus = Reg(init=reset_mstatus)
  /* When a trap is taken into M-mode, 
   * mepc is written with the virtual address of the instruction that
  ...
  /* When a trap is taken into M-mode, 
   * mtval is written with exception-specific information 
  ...
  /* "I" means RV32I/64I/128I base ISA, here xlen = 32 */
 val isa_string = "I"
 ...
 // csr register read/write accessibility
 val read_only = io.decode.csr(11,10).andR
 // cpu general register write enable
 ...
 // privilege too low or no such csr register or 后面的条件是什么意思？
 ...
 when(insn_call){
   ...
   // 这边为什么没有保存mepc？mret时怎么知道该返回到哪里执行？
 }
